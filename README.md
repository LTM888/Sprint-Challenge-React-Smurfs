...
The main differences between the two are how they handle a request is handled. With server-side routing, a whole new document is served to the user. On the other hand with client-side routing, the route is handled internally by JavaScript that reloaded the page.
